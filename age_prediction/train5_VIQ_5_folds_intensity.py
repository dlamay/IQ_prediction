...
from .dataset3 import AgePredictionDataset
from .vgg import VGG8

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
SPLITS_DIR = os.path.join(ROOT_DIR, "folderlist4")
...
def load_samples(split_fnames, bin_width=1, min_bin_count=10, max_samples=None):
    ...
    schema = {'id': str, 'iq': float, 'sex': str, 'path': str}
    dfs = []
    for fname in split_fnames:
        df = pd.read_csv(fname, sep=' ', header=None, names=['id', 'iq', 'sex', 'path'], dtype=schema)
        print(df)
        df['path'] = df['path'].apply(correct_path)
        df['iqbin'] = df['iq'] // bin_width
        dfs.append(df)
    ...
    if min_bin_count is not None:
        bin_counts = combined_df['iqbin'].value_counts()
        bins_below_cutoff = [bin for bin in bin_counts.keys() if bin_counts[bin] < min_bin_count]
        combined_df = combined_df[~combined_df['iqbin'].isin(bins_below_cutoff)]
...
def resample(df, mode, oversamp_limit):
    ...
    bins = sorted(set(df['iqbin']))
    bin_counts = {bin: sum(df['iqbin'] == bin) for bin in bins}
    ...
    if mode == 'none':
        ...
        for bin in bins:
            ...
            if n_under <= 0:
                ...
                new_samples = df[df['iqbin'] == bin].sample(oversamp_limit, replace=False)
                df = df[df['iqbin'] != bin]
                df = pd.concat([df, new_samples], axis=0)
            else:
                # Oversample as usual
                new_samples = df[df['iqbin'] == bin].sample(n_under, replace=True)
                df = pd.concat([df, new_samples], axis=0)
    elif mode == 'under':
        for bin in bins:
            n_over = bin_counts[bin] - undersamp_limit
            assert n_over >= 0
            new_samples = df[df['iqbin'] == bin].sample(undersamp_limit, replace=False)
            df = df[df['iqbin'] != bin]
            df = pd.concat([df, new_samples], axis=0)
        ...
        for bin in bins:
            ...
            new_samples = df[df['iqbin'] == bin].sample(count, replace=replace)
            df = df[df['iqbin'] != bin]
            df = pd.concat([df, new_samples], axis=0)
    else:
...
def setup_model(arch, device):
    if arch == 'resnet18':
        ...
        model = VGG8(in_channels=150, num_classes=1)
    else:
...
def main():
    ...
    '''
    split_fnames = glob.glob(f"{SPLITS_DIR}/FIQ_train_fold_*.list")
    assert len(split_fnames) == 5

    df = load_samples(split_fnames, opts.bin_width, opts.min_bin_count, opts.max_samples)
    _train_df, val_df = train_test_split(df, test_size=0.2, stratify=df['iqbin'])
    '''
    data_list = glob.glob(f"{SPLITS_DIR}/all_viq_data_new.list")
    df = load_samples(data_list, opts.bin_width, opts.min_bin_count, opts.max_samples)
    ...
    for fold in kf5.split(df):
        ...
        train_df = resample(training, mode = opts.sample, oversamp_limit = opts.oversamp_limit)

        if opts.print_bin_counts:
            with pd.option_context('display.max_rows', None, 'display.max_columns', None):
                log(df['iqbin'].value_counts())
                log(_train_df['iqbin'].value_counts())
                log(train_df['iqbin'].value_counts())
                log(val_df['iqbin'].value_counts())
            sys.exit(0)
        ...
        val_dataset = AgePredictionDataset(validation)
        ...
        with open(f"{results_dir}/config.json", 'w+') as f:
            ...
            cfg['train_counts'] = json.loads(train_df['iqbin'].value_counts().to_json())
            cfg['val_counts'] = json.loads(validation['iqbin'].value_counts().to_json())
        ...
        if opts.eval is None:
            ...
            for epoch in range(opts.n_epochs):
                ...
                if val_loss < best_val_loss:
                    torch.save(model.state_dict(), f"{checkpoint_dir}/best_model_f{i}.pth")
                    best_epoch = epoch
        ...
        checkpoint = torch.load(f"{checkpoint_dir}/best_model_f{i}.pth", map_location=device)
        ...
        val_df_with_preds.to_csv(f"{results_dir}/best_model_val_preds_f{i}.csv", index=False)
        i = i + 1
